...
const MyOrders = () => {
	...
	useEffect(() => {
		fetch(
			`https://manufacture-online-server.herokuapp.com/orders?username=${displayName}&email=${email}`,
			{
				headers: {
	...
	const handleCancel = () => {
		...
		fetch("https://manufacture-online-server.herokuapp.com/cancel-order", {
			...
			setOrders(orders.filter((el) => el._id !== cancelOrder._id))
		);
	};
